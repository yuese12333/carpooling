...
| 能力 | 接口 | 说明 |
|------|------|------|
| 发码（注册） | `POST /api/sms/send-verify-code` | 阿里云 SDK，详见 [短信验证接口联调文档.md](./短信验证接口联调文档.md) |
| 验码 + 业务令牌 | `POST /api/sms/check-verify-code` | 阿里云验码通过后签发 `tempToken` |
| 其它场景发码 | `POST /api/auth/sms/send` | Redis 自管验证码（登录/重置密码等），**注册不用此接口** |
...
sequenceDiagram
  ...
  App->>Sms: POST send-verify-code
  App->>Sms: POST check-verify-code
...
| 2 | POST | `/api/sms/send-verify-code` | 阿里云发码（请求体同 [短信验证接口联调文档.md](./短信验证接口联调文档.md)） |
| 3 | POST | `/api/sms/check-verify-code` | 阿里云验码 + 返回 `tempToken` |
...
- **POST** `/api/sms/check-verify-code`
...
| POST | `/api/auth/register/verify-code` | **已移除**，请用 `/api/sms/check-verify-code` |

## 7. 前端对接参考

- API：`frontend/src/api/auth.ts`（`sendSmsCode` → `/sms/send-verify-code`，`verifySmsCode` → `/sms/check-verify-code`）
